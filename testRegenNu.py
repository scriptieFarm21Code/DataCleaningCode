...
cursor.execute(f"SELECT readings.*, raw.weather_data FROM readings JOIN raw_readings as raw ON readings.raw_reading_id = raw.id WHERE raw.timestamp < 1588809600 AND raw.timestamp > 1588334400 AND readings.sensor_id = 100 AND raw.valid_reading = 1 AND readings.measured_at > 0 AND readings.soil_moisture_30 > 8 AND readings.latitude <> 0 AND readings.longitude <> 0 AND raw.weather_data IS NOT NULL AND readings.soil_moisture_10 <100 ORDER BY measured_at ASC")
...
allData['temp'] = pd.to_numeric(allData['temp'])
allData['apparentTemperature'] = pd.to_numeric(allData['apparentTemperature'])
allData['airPressure'] = pd.to_numeric(allData['airPressure'])
allData['dewPoint'] = pd.to_numeric(allData['dewPoint'])
allData['rain'] = pd.to_numeric(allData['rain'])
